...
namespace SteamQuery.Helpers
{
    public static class IpHelper
    {
        ...
        {
            if (!endPoint.Contains(':') || endPoint.Count(c => c == ':') > 1) // If there is no colon or more than one in the input.
            {
            ...
            if(!IPAddress.TryParse(ep[0], out var ip)) // If it's not a valid IP address, it might be a hostname, like: play.somehostname.com
            {
