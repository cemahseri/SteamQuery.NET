...
namespace SteamQuery
{
    ...
    public sealed class Server : IDisposable
    {
        ...
        // So, instead of that, I had seperated two methods and used asynchronous methods on the asynchronous method, such as UdpClient.SendAsync (instead of Send), UdpClient.ReceiveAsync (Receive), and etc.
        private byte[] ExecuteQuery(byte[] query)
        {
            _udpClient.Send(query, query.Length);
            var response = _udpClient.Receive(ref _ipEndPoint);
            
            var queryIdentifier = query[4];
            switch (queryIdentifier)
            {
                ...
                case 0x55: // Players
                case 0x56: // Rules
                    response[4] = queryIdentifier;
                    break;
                default:
                    throw new UnexpectedByteException(queryIdentifier, 0x54, 0x55, 0x56);
            }
